...
function createFileFromChunks($temp_dir, $fileName, $chunkSize, $totalSize,$total_files) {
    ...
    $temp_total = 0;
    foreach(getDirContents($temp_dir) as $file) {
        $temp_total = $total_files_on_server_size;
        $tempfilesize = filesize($file);
...
if (!empty($_FILES)) foreach ($_FILES as $file) {
    ...
    if (!move_uploaded_file($file['tmp_name'], $dest_file)) {
        ...
        isset($_POST['resumableRelativePath']) ? $_POST['resumableRelativePath'] : $_POST['resumableFilename'],
